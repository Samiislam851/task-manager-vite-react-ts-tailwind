...
const InputField = ({ task, setTask }: props) => {
  ...
  return (
    <form onSubmit={handleAddButton} className='mx-auto w-fit my-10 relative'>
      <input value={task} onChange={(e) => setTask(e.target.value)} type="text" className="transition-all rounded-full ease-in-out duration-300 p-3 px-4 border-0 task-input focus:shadow-2xl" placeholder='Enter you task here    ' />
